        ...
        public async void NapDanhSachDonHang(List<DonHang> donHangs, double heSoSong, bool napViTri1 = true)
        {
            try
            {
                if (DriverConnector.IsStarted &&
                    ...
                    donHangs != null && MayMenTags.Instance.STT1 != null)
                {
                    if (long.TryParse(MayMenTags.Instance.STTMat1.Value, out long sttMat))
                    {
                        ...
                        List<DonHang> donHangMat = donHangs.Where(x =>
                            x.HoanTatMayMen == 0 && x.STT >= sttMat).OrderBy(x => x.STT).ToList();
                            ...
                            long.TryParse(CutterTags.Instance.STT1.Value, out long sttCutter))
                        {
                            ...
                            DonHang donHang = dhs.FirstOrDefault(x => x.HoanTatMayMen == 0 && x.STT >= stt);
