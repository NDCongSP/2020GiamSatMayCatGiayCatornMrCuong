...
namespace MayCatServer
{
    public class Matrix
    {
        ...
        public string Area1Text = "";// nội dung
        ...
        public int Area4FontSize = 30; // cỡ chữ
        ...
        #endregion
        private void addArea(int _nProgramID, int _AreaX, int _AreaY, int _AreaWidth, int _AreaHeight, IntPtr _pPath, IntPtr _pNULL, int _nErrorCode, string _AreaText, string _AreaFontName, int _AreaFontSize, int _nTextColor, int _AreaStype, int _AreaShowEffect, int _AreaShowSpeed, string _donvi)
        {
            #region nap phan vung
            // thêm phân vùng 1
            int nAreaID = CSDKExport.Hd_AddArea(_nProgramID, _AreaX, _AreaY, _AreaWidth, _AreaHeight, _pPath, 3, 5, _pNULL, 0);
            if (nAreaID == -1)
            {
        ...
        public void Sendata(out int errorCode)
        {
            errorCode = -1;
            try
            {
                ...
                // Thêm vùng realtime
                //CSDKExport.Hd_CreateRealtimeArea(Area4X, Area4Y, Area4Width, Area4Height, pNULL, 1, 0, 0, 0, pNULL, 0);
                ...
                if (nRe != 0)
                {
                    errorCode = CSDKExport.Hd_GetSDKLastError();
        ...
        public void SendRealTimeData(string text)
        {
            int nTextColor = CSDKExport.Hd_GetColor(255, 0, 0);
            m_pSendParams = Marshal.StringToHGlobalUni(ip);
            ...
            IntPtr pFontName = Marshal.StringToHGlobalUni(Area4FontName);
