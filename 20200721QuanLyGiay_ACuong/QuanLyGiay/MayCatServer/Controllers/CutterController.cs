...
namespace MayCatServer
{
    public class CutterController
    {
        ...
        public async void NapDanhSachDonHang(List<DonHang> donHangs, bool napViTri1, bool napViTri2)
        {
            await semaphore.WaitAsync();
            try
            {
                if (donHangs != null && DriverConnector.IsStarted)
                {
                    if (long.TryParse(CutterTags.Instance.STT1.Value, out long stt))
                    {
                        //await CutterTags.Instance.LenhChuyenDon?.WriteAsync("0");
                        // Lọc các đơn chưa hoàn thành cutter và có stt lớn hơn STT đang chạy
                        List<DonHang> DanhSachNap = donHangs.Where(
                            x => x.HoanTatCutter == 0 && x.STT >= stt).OrderBy(x => x.STT).ToList();
        ...
        public async void CommitWrite(int soLanThuToiDa = 1)
        {
