...
namespace MayCatServer
{
    public class SongBController
    {
        ...
        public async void NapDanhSachDonHang(List<DonHang> donHangs, double heSoSong, bool napViTri1 = true)
        {
            try
            {
                    ...
                    donHangs != null)
                {
                    if (long.TryParse(SongBTags.Instance.STTSong1.Value, out long sttSong))
                    {
                        ...
                        List<DonHang> donHangSong = donHangs.Where(x =>
                             x.HoanTatGiaySongB == 0 && x.STT >= sttSong).OrderBy(x => x.STT).ToList();
                        ...
                        if (long.TryParse(SongBTags.Instance.STTMat1.Value, out long sttMat))
                        {
                            List<DonHang> donHangMat = donHangs.Where(x =>
                                x.HoanTatGiayMatB == 0 && x.STT >= sttMat).OrderBy(x => x.STT).ToList();
                                ...
                                long.TryParse(CutterTags.Instance.STT1.Value, out long sttCutter))
                            {
                                ...
                                DonHang donHang = dhs.FirstOrDefault(x => x.HoanTatSongB == 0 && x.STT >= stt);
        ...
        public void NapDonHang(List<DonHang> donHangs, long sttDon, bool napViTri1 = true)
        {
            if (long.TryParse(CutterTags.Instance.STT1.Value, out long sttCutter) &&
                long.TryParse(SongBTags.Instance.STT1.Value, out long sttDon1))
            {
